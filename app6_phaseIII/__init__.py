...
from app3_testII import MarketWaitPage
...
class Choice(Page):
    ...
    def before_next_page(player, timeout_happened):
        ...
        return choice_before_next_page(player, C.NUM_ROUNDS, timeout_happened, current_phase = 'III', distance=True, AUTO_FEE=C.AUTO_FEE, vary=True)
...
class Results(Page):
    ...
    @staticmethod
    def vars_for_template(player):
        return results_vars_for_template(player, C.NUM_ROUNDS, C.TRANSACTION_COSTS, C.INITIAL_PRICE, distance=True, current_phase='III', PRICE_CHANGE_RATE=C.PRICE_CHANGE_RATE)
